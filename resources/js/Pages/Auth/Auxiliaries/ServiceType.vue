...
const deleteserviceType = (id) => {
    isLoader.value = true;
    formDelete.id = id;
    formDelete.delete(route('service-types-delete'), {
        onFinish: () => {
...
const edit = (serviceType) => {
    inEdition.value = serviceType.id;
    form.name = serviceType.name;
    form.id = serviceType.id;
};

const submit = () => {
    form.post(route('service-type-save'), {
        onSuccess: () => {
            form.reset();
            inEdition.value = 0;
        },
...
const activate = (id) => {
    isLoader.value = true;
    form.put(route('service_types-activate', id), {
        onFinish: () => {
            isLoader.value = false;
        },
    });
...
const deactivate = (id) => {
    isLoader.value = true;
    form.put(route('service_types-deactivate', id), {
        onFinish: () => {
            isLoader.value = false;
        },
    });
                                        ...
                                        <tbody>
                                            <tr v-for="(serviceType, index) in serviceTypes"
                                                ...
                                                <td>
                                                    ...
                                                    <Modal :key="index"
                                                        :modal-title="'Confirmar Exclusão de ' + serviceType.name"
                                                        :ok-botton-callback="deleteserviceType"
                                                        :ok-botton-callback-param="serviceType.id"
                                                    ...
                                                    <button v-if="!serviceType.active" class="btn btn-sm btn-success btn-icon-split mr-2"
                                                        v-on:click="activate(serviceType.id)">
                                                        <span class="icon text-white-50">
                                                    ...
                                                    <button v-if="serviceType.active" class="btn btn-sm btn-warning btn-icon-split mr-2"
                                                        v-on:click="deactivate(serviceType.id)">
                                                        <span class="icon text-white-50">
