...
const deleteVehicle = (id) => {
    isLoader.value = true;
    formDelete.id = id;
    formDelete.delete(route('vehicle-delete'), {
        onFinish: () => {
...
const edit = (vehicle) => {
    inEdition.value = vehicle.id;
    form.name = vehicle.name;
    form.id = vehicle.id;
};

const submit = () => {
    form.post(route('vehicle-save'), {
        onSuccess: () => {
            form.reset();
            inEdition.value = 0;
        },
...
const activate = (id) => {
    isLoader.value = true;
    form.put(route('vehicles-activate', id), {
        onFinish: () => {
            isLoader.value = false;
        },
    });
...
const deactivate = (id) => {
    isLoader.value = true;
    form.put(route('vehicles-deactivate', id), {
        onFinish: () => {
            isLoader.value = false;
        },
    });
                                    ...
                                    <table class="table table-striped" id="dataTable" width="100%" cellspacing="0">
                                        ...
                                        <tbody>
                                            <tr v-for="(vehicle, index) in vehicles"
                                                :class="{ 'table-info': inEdition == vehicle.id }">
                                                <th scope="row">{{ vehicle.id }}</th>
                                                <td>{{ vehicle.name }}</td>
                                                <td>
                                                    <button class="btn btn-sm btn-info btn-icon-split mr-2"
                                                        v-on:click="edit(vehicle)">
                                                    ...
                                                    <Modal :key="index"
                                                        :modal-title="'Confirmar Exclusão de ' + vehicle.name"
                                                        :ok-botton-callback="deleteVehicle"
                                                        :ok-botton-callback-param="vehicle.id"
                                                    ...
                                                    <button v-if="!vehicle?.active" class="btn btn-sm btn-success btn-icon-split mr-2"
                                                        v-on:click="activate(vehicle.id)">
                                                        <span class="icon text-white-50">
                                                    ...
                                                    <button v-if="vehicle?.active" class="btn btn-sm btn-warning btn-icon-split mr-2"
                                                        v-on:click="deactivate(vehicle.id)">
                                                        <span class="icon text-white-50">
