...
const deleteRegime = (id) => {
    isLoader.value = true;
    formDelete.id = id;
    formDelete.delete(route('regime-delete'), {
        onFinish: () => {
...
const edit = (regime) => {
    inEdition.value = regime.id;
    form.name = regime.name;
    form.id = regime.id;
};

const submit = () => {
    form.post(route('regime-save'), {
        onSuccess: () => {
            form.reset();
            inEdition.value = 0;
        },
    ...
    <AuthenticatedLayout>
        ...
        <template #header>
            <div class="d-sm-flex align-items-center justify-content-between">
                <h1 class="h3 mb-0 text-gray-800">Regime</h1>
            </div>
        </template>
        <div class="row">
            <div class="col-lg-12">
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table class="table table-striped" id="dataTable" width="100%" cellspacing="0">
                                        ...
                                        <tbody>
                                            <tr v-for="(regime, index) in regimes"
                                                :class="{ 'table-info': inEdition == regime.id }">
