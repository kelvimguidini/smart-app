...
import { ProviderAirfareService, ProviderAirfare, ProviderAirfareCreateUpdateRequest } from '../../../services/provider-airfare.service';
...
@Component({
  ...
  templateUrl: './provider-airfare.component.html',
  styleUrls: ['./provider-airfare.component.scss'],
})
export class ProviderAirfareComponent implements OnInit {
  private readonly providerAirfareService: ProviderAirfareService = inject(ProviderAirfareService);
  private readonly cityService: CityService = inject(CityService);
  private readonly toastService: ToastService = inject(ToastService);

  providers: ProviderAirfare[] = [];
  inEdition: number = 0;
  ...
  loadProviders(): void {
    ...
    this.providerAirfareService.getProviders(params).subscribe({
      ...
      error: (error: HttpErrorResponse) => {
        this.isLoader = false;
        this.toastService.error('Erro ao carregar fornecedores de aéreo');
        console.error('Erro ao carregar fornecedores de aéreo:', error);
      },
    });
  }
  ...
  edit(provider: ProviderAirfare): void {
  ...
  submit(): void {
    ...
    const data: ProviderAirfareCreateUpdateRequest = {
      id: this.form.id,
      name: this.form.name,
    ...
    this.providerAirfareService.saveProvider(data).subscribe({
      next: (response: any) => {
        ...
        this.toastService.success(response.message || 'Fornecedor de aéreo salvo com sucesso');
        this.closeModal();
        this.loadProviders();
      },
      error: (error: HttpErrorResponse) => {
        ...
        if (error.status === 422) {
          ...
          this.toastService.error('Erro ao salvar fornecedor de aéreo');
        }
        console.error('Erro ao salvar fornecedor de aéreo:', error);
      },
    });
  }
  ...
  deleteProvider(id: number): void {
    ...
    this.providerAirfareService.deleteProvider(id).subscribe({
      next: (response: any) => {
        this.isLoader = false;
        this.toastService.success(response.message || 'Fornecedor de aéreo apagado com sucesso');
        this.loadProviders();
      },
      error: (error: HttpErrorResponse) => {
        this.isLoader = false;
        this.toastService.error('Erro ao apagar fornecedor de aéreo');
        console.error('Erro ao deletar fornecedor de aéreo:', error);
      },
    });
  }
  ...
  activateProvider(id: number): void {
    ...
    this.providerAirfareService.activateProvider(id).subscribe({
      next: (response: any) => {
        this.isLoader = false;
        this.toastService.success(response.message || 'Fornecedor de aéreo ativada com sucesso');
        this.loadProviders();
      },
      error: (error: HttpErrorResponse) => {
        this.isLoader = false;
        this.toastService.error('Erro ao ativar fornecedor de aéreo');
        console.error('Erro ao ativar fornecedor de aéreo:', error);
      },
    });
  }
  ...
  deactivateProvider(id: number): void {
    ...
    this.providerAirfareService.deactivateProvider(id).subscribe({
      next: (response: any) => {
        this.isLoader = false;
        this.toastService.success(response.message || 'Fornecedor de aéreo inativado com sucesso');
        this.loadProviders();
      },
      error: (error: HttpErrorResponse) => {
        this.isLoader = false;
        this.toastService.error('Erro ao inativar fornecedor de aéreo');
        console.error('Erro ao inativar fornecedor de aéreo:', error);
      },
    });
  }
