...
class CurrencyController extends Controller
{
    ...
    {
        if (!Gate::allows('currency_admin')) abort(403);
    ...
    public function create()
    {
        ...
        return Inertia::render('Auth/Auxiliaries/Currency', [
            'currencies' => $this->lookupRepository->getCurrenciesWithInactive()
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $this->lookupRepository->saveCurrency($data, $request->id > 0 ? $request->id : null);
        } catch (\Exception $e) {
            throw $e;
        }

        return redirect()->route('currency')->with('flash', ['message' => 'Registro salvo com sucesso', 'type' => 'success']);
    }

    public function delete(Request $request)
    {
        if (!Gate::allows('currency_admin')) abort(403);
        $this->lookupRepository->deleteCurrency($request->id);
        return redirect()->route('currency')->with('flash', ['message' => 'Registro apagado com sucesso!', 'type' => 'success']);
    }
}
